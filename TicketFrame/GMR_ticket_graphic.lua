...
function GMR.FillTicketFrame()
	...
	b7 = GMR.CreateButton("CloseTicket",  offs_x-10, offs_y-35, 120, 25, GMR.TicketFrame, "TOPLEFT", "TOPLEFT", "Close Ticket", "Close this ticket")
	...
	b5_1 = GMR.CreateButton("UnAssignTicket",  offs_x-10, offs_y+55, 120, 25, GMR.TicketFrame, "TOPLEFT", "TOPLEFT", "Unassign", {"Unassign this ticket.", ""})
...
--	)
	...
	e3 = GMR.CreateEditBox("GMAssignName", offs_x+148, offs_y+90, GMR.TicketFrame, 110, 25, "TOPLEFT", "TOPLEFT", "Assign the ticket to specified GM")

	b5 = GMR.CreateButton("AssignTicket",  offs_x-10, offs_y+85, 120, 30, GMR.TicketFrame, "TOPLEFT", "TOPLEFT", "Assign", {"Give this ticket to anybody.", " If textbox empty - assign to yourself."})
	b5:SetScript("OnClick",
		function()
			if GMR.CurrentTicketInfo and GMR.CurrentTicketInfo.num then 
				...
				if assigned_person == "" 
					then assigned_person = UnitName("player")
				end
				SendChatMessage(".ticket assign "..GMR.CurrentTicketInfo.num.." "..assigned_person, "GUILD")
				if assigned_person == UnitName("player") then
					GMR.Print(SLib.GetRandomColor().."Assigned to "..assigned_person.."\124r") 
				end
			GMR.EditBoxes["GMAssignName"]:SetText("");
			end
		end
	)
	b8 = GMR.CreateButton("GotoTicketerCreator",  offs_x+140, offs_y-35, 120, 25, GMR.TicketFrame, "TOPLEFT", "TOPLEFT", "Go to Player", "")
	...
	local b8_2 = GMR.CreateButton("SummonCreator",  offs_x+140, offs_y-5, 120, 25, GMR.TicketFrame, "TOPLEFT", "TOPLEFT", "Summon Player", "")
	...
	b8_1 = GMR.CreateButton("BanInfoCreator",  offs_x+140, offs_y+25,  120, 25, GMR.TicketFrame, "TOPLEFT", "TOPLEFT", "Ban Info", "")
	...
	local b8_3 = GMR.CreateButton("PlayerInfo",  offs_x+140, offs_y+55, 120, 25, GMR.TicketFrame, "TOPLEFT", "TOPLEFT", "Player Info", "")
...
--[[	local b8_4 = GMR.CreateButton("LookupPlayerIP",  offs_x+260, offs_y+85, 50, 25, GMR.TicketFrame, "TOPLEFT", "TOPLEFT", "Lookup by IP", "")
	b8_4:SetScript("OnClick",
		function()
			local pin
			if GMR.CurrentTicketInfo and GMR.CurrentTicketInfo.creator then
				pin=RunLine(".pinfo "..GMR.CurrentTicketInfo.creator)
				Print(pin)
...
--	)
	...
	local oAss2 = GMR.CreateButton("oAssRank3",  offs_x-10, offs_y-5, 120, 25, GMR.TicketFrame, "TOPLEFT", "TOPLEFT", "Rank 3 GM", "")
	oAss2:SetScript("OnClick",
		function()
			if GMR.CurrentTicketInfo and GMR.CurrentTicketInfo.creator then 
				SendChatMessage(".ticket assign "..GMR.CurrentTicketInfo.num.." Rankthree", "GUILD")
			end
		end
	)

	local oAss3 = GMR.CreateButton("oAssRank4",  offs_x-10, offs_y+25, 120, 25, GMR.TicketFrame, "TOPLEFT", "TOPLEFT", "Rank 4 GM", "")
	...
	b2 = GMR.CreateButton("TicketSendMailResponce",  offs_x+125, offs_y+305, 80, 25, GMR.TicketFrame, "TOPLEFT", "TOPLEFT", "Send Mail", "")
	...
	local sendItem = GMR.CreateButton("TicketSendItem",  offs_x+215, offs_y+305, 80, 25, GMR.TicketFrame, "TOPLEFT", "TOPLEFT", "Send Item", "")
...
--end
	...
	local restoreChar = GMR.CreateButton("TicketRestoreCharacter", offs_x+220, offs_y+320, 80, 25, GMR.TicketFrame, "TOPLEFT", "TOPLEFT", "Restore GUID", "")
		restoreChar:SetScript("OnClick",
			function()
				if GMR.CurrentTicketInfo and GMR.CurrentTicketInfo.num then 
					local charGUID = GMR.EditScrollBoxes["TicketMailAnswer"]:GetText();
					if charGUID == "" then
						DEFAULT_CHAT_FRAME:AddMessage("|cffFF0000GMR: Please use the window below Send Item to enter an item number.|r");
--					elseif not tonumber(charGUID) then
--						DEFAULT_CHAT_FRAME:AddMessage("|cffFF0000GMR: Item number must be numeric.|r");
					elseif charGUID == nil then
						DEFAULT_CHAT_FRAME:AddMessage("|cffFF0000GMR: Invalid Item ID number entered.|r");
					else					
						SendChatMessage(".character deleted restore "..charGUID, "GUILD")
						SendChatMessage(".ban note "..GMR.CurrentTicketInfo.creator.." \"Restored character with GUID "..charGUID..".\"", "GUILD")
					end
				end
			end
		)
...
function GMR.GetAccountName(charName)
  GMR.SendCommandSafe(".baninfo "..charName, "baninfo")
end
